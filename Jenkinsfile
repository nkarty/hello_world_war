    ...
    stages {
        stage('Clone Step') {
            steps {
                ...
                sh 'git clone https://github.com/nkarty/hello_world_war.git'
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                withDockerRegistry([ credentialsId: "docker-hub-creds", url:""]) {
                    sh 'docker tag mvn_docker karty94/mvn_docker:$BUILD_NUMBER'
                    sh 'docker push karty94/mvn_docker:$BUILD_NUMBER' 
                }
            }
        }
        stage('Deploy Step') {
